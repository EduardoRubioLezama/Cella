...
drop table if exists cella.unidadMaterial cascade;

create table cella.unidadMaterial(
	id serial primary key,
	nombreMaterial text not null unique,
...
drop table if exists cella.solicitarPrestamoProfesor cascade;

create table cella.solicitarPrestamoProfesor(
	id serial primary key,	
	id_unidad_material int references cella.unidadMaterial(id) on delete cascade,
	fechainicio date not null
);

drop table if exists cella.solicitarPrestamoAlumno cascade;

create table cella.solicitarPrestamoAlumno(
	id serial primary key,	
	id_unidad_material int not null references cella.unidadMaterial(id) on delete cascade,    
	fechainicio date not null
);
